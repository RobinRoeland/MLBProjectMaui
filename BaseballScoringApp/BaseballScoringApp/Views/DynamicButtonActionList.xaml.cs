    ...
    public partial class DynamicButtonActionList : Popup
    {
        ...
        public double CalculatedHeight
        {
            get
            {
                if (the_actions != null)
                {
                    MaxPopupHeight = DeviceDisplay.MainDisplayInfo.Height * 0.8 / DeviceDisplay.MainDisplayInfo.Density;
                    double itemHeight = 50; // action text height in collectionview
                    double wantedMinHeight = Math.Max(the_actions.Count * itemHeight + 20, 100);
        ...
        public DynamicButtonActionList(List<IGameAction> actions)
        {
            ...
            TitleText.Text = "Action for \n" + action.playerInvolved.GetShortDisplayString();
